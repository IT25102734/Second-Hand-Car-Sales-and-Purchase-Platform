...
public class UserService {
    ...
    // Helper: create correct User subclass from CSV line (handles extra fields)
    private static User createUserFromLine(String line) {
        ...
        try {
            ...
            if ("buyer".equals(userType) && parts.length >= 9) {
                ...
                // Basic user (should not happen in normal flow)
    ...
    public boolean registerUser(User user) throws IOException {
        ...
        user.setId(newId);
        // Ensure isAdmin is false for new registrations (you can later set manually)
        user.setAdmin(false);
